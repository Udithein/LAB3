                   ...
                   int nCmdShow)
{
    ...
    Load_Textur("background.png", &bg, GL_CLAMP, GL_CLAMP, GL_NEAREST);
    /* program main loop */
    ...
    while (!bQuit)
    {
        ...
        else
        {
            ...
            None = new Button("Play", 150, 50, new Point(windowWidth/2, windowHeight/2)); None->Show();
            ...
            if(hero->HeroStatement!=7)
                ...
                {
                    hero->HeroStatement=0;
                }

            if (hero->position->x-5>=0)
                {
                    hero->position->x-=5;
                    Sleep(40);
                }
            ...
            if (GetKeyState(VK_UP) & 0x8000 //&& hero->position->y==300
                )
                {
    ...
    switch (uMsg)
    {
        ...
        case WM_LBUTTONDOWN:
            if(windowNumber == 1){
                ...
                {
                    std::cout <<  SetWindowPos (hwnd, HWND_TOP, 0, 0, 1280, 512, SWP_SHOWWINDOW);
                    windowNumber = 0;
                }
                if (EventHandler(LOWORD(lParam), HIWORD(lParam), None))
                {
                    std::cout << None->name;
                    SetWindowPos (hwnd, HWND_TOP, 0, 0, 1280, 512, SWP_SHOWWINDOW);
                    windowNumber = 2;
                }
                ...
                {
                    windowNumber = 1;
                    SetWindowPos (hwnd, HWND_TOP, 0, 0, 512, 512, SWP_SHOWWINDOW);
                }
            }
